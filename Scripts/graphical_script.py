...
def guibuild():
    ...
    link = "https://github.com/Bhuvek"
    text = Text(root, height=1, padx=10, borderwidth=1, bg="black")
    ...
    labe_head = Label(root, text="LinkedIn Connection Bot", fg="black", font=fontstyle).grid(row=1, column=0)
    labe_des = Label(root, text="WELCOME", fg="black",font=fontstyle).grid(row=2,column=0)
    ...
    label = Label(root, text="Enter your intrests separated by ;").grid(row=11, column=0)
    label2 = Label(root, text="Search for TCS , Accenture and Wipro Enter: ").grid(row=12, column=0)
    ...
    labelx = Label(root, text="How many pages you want to Automate ").grid(row=18,column=0)
    e4.insert(0, constants.upto_page)
    e4.grid(row=20, column=0, columnspan=3, padx=10, pady=7)
    label_username = Label(root, text="Excel File will be Exported of Connected Users").grid(
        row=22, column=0)

    buttonsubmit = Button(root, text="START", fg='blue', height=5, width=40,
    ...
    link = "Thank you for Using this bot!"
    text = Text(root, height=1, padx=10, borderwidth=1, bg="black")
    text.insert(1.0, link)
    text.configure(state="normal")
